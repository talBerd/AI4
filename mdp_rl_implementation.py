...
def value_iteration(mdp: MDP, U_init: np.ndarray, epsilon: float=10 ** (-3)) -> np.ndarray:
    ...
    while delta >= epsilon * (1 - mdp.gamma) / mdp.gamma:
        ...
        # Iterate over all states in the MDP grid
        for row in range(mdp.num_row):
            for col in range(mdp.num_col):
                ...
                if (row, col) in mdp.terminal_states:
                    ...
                    # Get expected utility for each action
                    utilities = []
...
def get_policy(mdp: MDP, U: np.ndarray) -> np.ndarray:
    # Initialize an empty policy grid
    policy = [[None for _ in range(mdp.num_col)] for _ in range(mdp.num_row)]

    # Define the possible actions for easy iteration
    actions = mdp.actions
...
def policy_evaluation(mdp: MDP, policy: np.ndarray) -> np.ndarray:
    ...
    while True:
        ...
        for row in range(mdp.num_row):
            for col in range(mdp.num_col):
                val = mdp.board[row][col]

                if val == "WALL" or (row, col) in mdp.terminal_states:
                    continue

                # Convert the action from policy to an Action enum
                step = Action(policy[row][col])  # Convert to Action enum
                ...
                U_update[row][col] = float(val) + mdp.gamma * expected_utility
        ...
        if delta == 0:  # Stop when delta is below a small threshold
            break
            ...
            for col in range(mdp.num_col):
                ...
                for action in Action:
                    ...
                    for i, direction in enumerate(Action):
                        ...
                        probability = mdp.transition_function[action][i]  # Access by index assuming ordering
                ...
                if best_action and best_action != current_action:
                    policy[row][col] = str(best_action)  # Store as string to match policy_init format
                    stable = False
...
def adp_algorithm(
    ...
    """
    # Initialize transition counts with zeros for all (action, actual_action) pairs
    transition_counts = {action: {act: 0 for act in actions} for action in actions}
    rewards = np.zeros((num_rows, num_cols, len(actions)))  # Initialize the rewards matrix
    ...
    for episode_gen in sim.replay(num_episodes):
        for state, reward, action, actual_action in episode_gen:
            if action is not None:
                ...
                rewards[row, col, action_index] = reward  # Set reward for the state-action pair
    ...
    for action in actions:
        ...
        if total_action_count > 0:
            ...
            for actual_action in actions:
                transition_probabilities[action][actual_action] = 0.0  # No occurrences of this action
